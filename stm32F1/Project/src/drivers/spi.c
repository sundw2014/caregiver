...
* Description    : Initializes the peripherals SPI1
* Input          : None
* Output         : None
* Return         : None
*******************************************************************************/
void SPI1_Init(void)
{
    ...
    /*!< Configure SPI_NCS_PIN pin: adns9800 NCS_PIN */
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_Init(GPIOA, &GPIO_InitStructure);

    /* Deselect the A9800: Chip Select high */
    SPI_A9800_CS_HIGH();
...
{
    motion[0] = A9800ReadReg(SPI_A9800_REG_Delta_X_L);
    motion[0] = A9800ReadReg(SPI_A9800_REG_Delta_X_H)<<8 | motion[0];
    motion[1] = A9800ReadReg(SPI_A9800_REG_Delta_Y_L);
    motion[1] = A9800ReadReg(SPI_A9800_REG_Delta_Y_H)<<8 | motion[1];
}
...
// while(1){
//   SPI_I2S_SendData(SPI1, 0x3F);
//   while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) != SET);
// }
    A9800WriteReg(SPI_A9800_REG_Power_Up_Reset, 0x5A);
    DelayMs(50);

    A9800ReadReg(SPI_A9800_REG_Motion);
    A9800ReadReg(SPI_A9800_REG_Delta_X_L);
    A9800ReadReg(SPI_A9800_REG_Delta_X_H);
    A9800ReadReg(SPI_A9800_REG_Delta_Y_L);
    A9800ReadReg(SPI_A9800_REG_Delta_Y_H);

    A9800UploadFiremware(firmwareA4, firmwareA4Length);

    DelayMs(10);

    A9800WriteReg(SPI_A9800_REG_LASER_CTRL0, A9800ReadReg(SPI_A9800_REG_LASER_CTRL0) & 0xf0 );

    DelayMs(1);

    myPrintf("ID=%d,",A9800ReadReg(0x00));
    myPrintf("Invert_ID=%d\r\n",A9800ReadReg(0x3F));
    // while(1);
}
...
void A9800WriteReg(u8 addr, u8 data)
{
    ...
    SPI_A9800_CS_HIGH();
    delayMicroseconds(120); // tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound
}

void A9800UploadFiremware(u8 *firmwareAddr, u16 firmwareLength)
{
// set the configuration_IV register in 3k firmware mode
    A9800WriteReg(SPI_A9800_REG_Configuration_IV, 0x02); // bit 1 = 1 for 3k mode, other bits are reserved

// write 0x1d in SROM_enable reg for initializing
    A9800WriteReg(SPI_A9800_REG_SROM_Enable, 0x1d);

// wait for more than one frame period
    DelayMs(10); // assume that the frame rate is as low as 100fps... even if it should never be that low

// write 0x18 to SROM_enable to start SROM download
    A9800WriteReg(SPI_A9800_REG_SROM_Enable, 0x18);

// write the SROM file (=firmware data)
    SPI_A9800_CS_LOW();
    A9800RW(SPI_A9800_REG_SROM_Load_Burst | 0x80); // write burst destination adress
...
const u16 firmwareA4Length = 3070;
